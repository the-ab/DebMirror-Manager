...
docker-compose/                               standalone image installations with/without nginx plus ENV documentation
...
The exact template paths are `docker-compose/compose.yaml`, `docker-compose/.env.example`, `docker-compose/compose.no-nginx.yaml`, and `docker-compose/.env.no-nginx.example`; local documentation is available in `docker-compose/README.md` and `docker-compose/README.de.md`.
...
`COMPOSE_PROFILES=mirror-http` enables nginx. Set `COMPOSE_PROFILES=` to run only the WebUI with the same Compose file.

**Dedicated no-nginx variant:**
...
Before first start, review at least `DATA_PATH`, `MIRROR_PATH`, WebUI bind address/port, `DMM_IMAGE_TAG`, and `APP_TIMEZONE`. `docker-compose/README.md` explains every variable, default, security implication, and operational command. Real `.env` files must use mode `0600` and must never be added to Git.

For a fresh installation, leave `APP_SECRET_KEY` empty. The container creates a secure persistent key under `DATA_PATH/data/app-secret.key`; then create the first administrator at `http://SERVER-IP:8111/setup`. During migration, copy the previous `APP_SECRET_KEY` before the first image start.

Both Compose files use the same project and container names and must not run simultaneously. Switch variants after `docker compose ... down`; data remains available when both variants use the same `DATA_PATH` and `MIRROR_PATH`.
...
With no new ZIP, `./update.sh --rebuild` only creates a backup and rebuilds the local installation.

### GHCR image installation

The image installation uses neither a release ZIP nor `update.sh`.
