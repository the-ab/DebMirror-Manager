...
- Sprache wird pro Benutzerkonto in SQLite gespeichert; bestehende Benutzer werden sicher mit Deutsch migriert.
- Benutzerbezogene Darstellung ergänzt: Hell, Dark Mode und automatische Browser-/Systemdarstellung werden pro Konto gespeichert.
- Globalen Darstellungswert aus den Systemeinstellungen entfernt; vorhandene Benutzer übernehmen beim Update den bisherigen Darstellungsmodus.
- Neue Seite **Persönliche Einstellungen** für alle angemeldeten Benutzer ergänzt.
- Theme-Umschalter in der Kopfzeile für Admin- und Lese-Benutzer freigegeben und auf das jeweilige Konto begrenzt.
- Benutzerverwaltung um individuelle Sprache und Darstellung beim Anlegen und Bearbeiten erweitert.
- Login und Ersteinrichtung um Sprachauswahl ergänzt; die Ersteinrichtung speichert Sprache und Darstellung direkt am ersten Administratorkonto.
- WebUI-Ausgabe einschließlich Navigation, Formulare, Status- und Flashmeldungen für Englisch lokalisiert; technische API-Werte, Pfade, Befehle und Joblogs bleiben unverändert.
- Dokumentationsansicht wählt automatisch die zur Benutzersprache passende Datei.
- `README.md` und `RELEASE_NOTES.md` sind jetzt die englischen Standarddateien.
- `README.de.md` und `RELEASE_NOTES.de.md` enthalten die deutschen Fassungen.
- Dockerfile, Updateprüfung und Update-Backup um beide deutschen Dokumentationsdateien erweitert.
- VERSION auf 0.1.79 gesetzt.
